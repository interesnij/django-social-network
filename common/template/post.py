...
def get_template_community_post(community, folder, template, request_user):
...
def get_permission_community_post(community, folder, template, request_user):
    if community.is_suspended():
...
def get_template_user_post(user, folder, template, request_user):
    if request_user.is_authenticated:
...
def get_permission_user_post(user, folder, template, request_user):
    if user.is_suspended():
        raise PermissionDenied('Ошибка доступа')
    elif user.is_blocked():
        raise PermissionDenied('Ошибка доступа')
