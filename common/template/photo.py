...
def get_permission_community_photo(list, folder, template, request_user, user_agent):
...
def get_permission_community_photo_detail(list, photo, folder, template, request_user, user_agent):
    ...
    if community.type[0] == "_":
        ...
        if request_user.is_no_phone_verified():
            template_name = "main/phone_verification.html"
        elif request_user.pk == photo.creator.pk:
            template_name = folder + "admin_" + template
        elif request_user.is_staff_of_community(community.pk):
            template_name = folder + template
        elif request_user.is_photo_manager():
...
def get_template_user_photo(list, folder, template, request_user, user_agent):
    user = list.creator
    if request_user.is_authenticated:
        update_activity(request_user, user_agent)
        if request_user.is_no_phone_verified():
...
def get_permission_user_photo(list, folder, template, request_user, user_agent):
...
def get_permission_user_photo_detail(list, photo, folder, template, request_user, user_agent):
    from common.check.user import check_user_can_get_list, check_anon_user_can_get_list
    user = list.creator
