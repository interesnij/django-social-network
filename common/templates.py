...
def get_template_community(list, folder, template, request_user, user_agent, staff):
    ...
    if request_user.type[0] == "_":
        ...
        if list.is_deleted():
            if request_user.is_administrator_of_community(community.pk) and list.community.pk == community.pk:
...
def get_template_anon_user(list, template, request_user, user_agent):
    ...
    if user.type[0] == "_":
        ...
        if list.is_deleted():
            template_name = "generic/u_template/anon_list_deleted.html"
        elif list.is_closed():
            template_name = "generic/u_template/anon_list_closed.html"
        elif list.is_suspended():
            template_name = "generic/u_template/anon_list_suspended.html"
    elif user.is_closed_profile():
