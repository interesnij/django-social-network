...
def get_notify(user, verb, attach):
    ...
    if attach[:3] == "pos":
        ...
        if post.community:
            ...
            block = ''.join([block, '<span data-pk="', str(creator.pk), '"list-pk="', str(post.get_list_pk()), '" data-uuid="', str(post.uuid), '"><div class="card-header">\
            ...
            <div class="like_pop"><span class="u_all_posts_likes pointer">Оценили: ', post.likes_count_ru(), '</span>\
            <span style="display: flex;margin-top: 10px;">', window_likes, '</span></div></span><span ', votes_on, ' class="dislike \
            ...
            </svg><span class="dislikes_count">', str(post.dislikes_count()), '</span></span><span class="dislike_window">\
            <div class="dislike_pop"><span class="u_all_posts_dislikes pointer">Не оценили: ', post.dislikes_count_ru(), '</span>\
            <span style="display: flex;margin-top: 10px;">', window_dislikes, '</span></div></span><span title="Комментарий" \
