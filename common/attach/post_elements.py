    ...
    for item in post.attach.split(","):
        if item[0] == "l":
            ...
            if item[:3] == "pho":
                ...
                block = ''.join([block, '<div style="flex-basis: 100%;" class="card"><div class="card-body" style="padding: 5px"><div style="display:flex"><figure><a class="ajax" href="', commuity.get_link(), '">', image, '</a></figure><div class="media-body" style="margin-left: 10px;"><a href="', community.get_link(), '" class="my-0 mt-1 ajax">', community.name, '</a><p>', community.description, '<br>Подписчиков: ', str(community.community_info.members), '</p></div></div></div></div>'])
                #except:
                #    pass
