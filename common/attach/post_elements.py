def get_post_attach(post, user):
    ...
    for item in post.attach.split(","):
        if item[:3] == "pho":
            ...
            block = ''.join([block, '<div class="music" data-path="/media/', music.file.url, '" data-duration="', music.duration, '" style="flex-basis: auto;width:100%;position: relative;"><div class="media" music-counter="', str(music_counter), '">', figure, '<div class="media-body" style="display: flex;"><h6 class="music_list_post music_title"><a>', music.title, '</a></h6>', span_btn, '</div></div></div>'])
            #except:
            #    pass
        elif item[:3] == "doc":
            try:
