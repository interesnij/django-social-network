        ...
        if item[0] == "l":
            if item[:3] == "lmu":
                ...
                try:
                    ...
                    block = ''.join([block, '<div style="flex-basis: 100%;" class="card"><div class="card-body" owner-pk="', str(creator.pk), '" postlist-pk="', str(list.pk), '" style="padding: 8px;padding-bottom: 0;"><div style="display:flex"><figure><a class="load_posts_list pointer"><img src="' + img_src + '" style="height:120px;" alt="image"></a></figure><div class="media-body" style="margin-left: 10px;"><h6 class="my-0 mt-1 load_posts_list pointer">', list.name, '</h6><p>Список записей <a style="vertical-align: baseline;" class="ajax underline" href="', creator.get_link(), '">', name, '</a><br>Записей: ', str(list.count_items()), '</p></div></div></div></div>'])
                except:
                    pass
            elif item[:3] == "ldo":
                try:
            ...
            if item[:3] == "pho":
                ...
                try:
                    ...
                    block = ''.join([block, '<div class="music" data-path="', music.file.url, '" data-duration="', music.get_duration(), '" style="flex-basis: auto;width:100%;position: relative;"><div class="media" music-counter="', str(music_counter), '">', music.get_s_image(), '<div class="media-body" style="display: flex;"><h6 class="music_list_post music_title"><a>', music.title, '</a></h6>', span_btn, '</div></div><div class="progress2"></div></div>'])
                except:
                    pass
            elif item[:3] == "doc":
                try:
