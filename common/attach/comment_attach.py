    ...
    for item in comment.attach.split(","):
        if item[:3] == "pho":
            ...
            try:
                ...
                block = ''.join([block, '<div class="music" data-path="', music.uri, '" data-duration="', music.duration, '" style="flex-basis: 100%;position: relative;"><div class="media" music-counter="0">', figure, '<div class="media-body" style="display: flex;"><h6 class="music_list_comment music_title"><a>', music.title, '</a></h6>', span_btn, '</div></div></div>'])
            except:
                pass
        elif item[:3] == "goo":
            try:
...
def get_c_comment_attach(comment, user):
    block = ''
    for item in comment.attach.split(","):
        if item[:3] == "pho":
            try:
