    ...
    if request.is_ajax() and form_post.is_valid():
        ...
        for object_id in connections:
            if object_id[0] == "c":
                ...
                message = Message.send_message(chat=chat, creator=request.user, parent=None, text=text)
                ...
                get_post_attach(request, new_post)
                get_post_message_processing(new_post)
                message = Message.get_or_create_chat_and_send_message(creator=request.user, user=user, text=text)
                new_post.post_message.add(message)
            else:
