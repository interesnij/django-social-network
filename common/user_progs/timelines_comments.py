...
def get_timeline_post_comments(user_pk):
    ...
    own_posts_query = Q(id__in=[post['post_id'] for post in posts], is_deleted=False, status=Post.STATUS_PUBLISHED)
...
def get_timeline_photo_comments(user_pk):
    ...
    own_photos_query = Q(id__in=[photo['photo_comment_id'] for photo in photos], is_deleted=False, is_public=True)
...
def get_timeline_good_comments(user_pk):
    ...
    own_goods_query = Q(id__in=[good['good_comment_id'] for good in goods], is_deleted=False, status=Good.STATUS_PUBLISHED)
...
def get_timeline_video_comments(user_pk):
    ...
    own_videos_query = Q(id__in=[video['video_comment_id'] for video in videos], is_deleted=False, is_public=True)
