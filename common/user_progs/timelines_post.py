...
def get_timeline_posts(user):
    ...
    u_select_related, c_select_related = ('creator', 'parent'), ('creator', 'community', 'parent')
    my_posts = user.post_creator.only('created').select_related(*u_select_related).filter(Q(creator_id=user.pk, community__isnull=True, is_deleted=False, status="P"))
    ...
    community_posts = Post.objects.only('created').select_related(*c_select_related).filter(_community_posts)
    ...
    followed_users = Post.objects.only('created').select_related(*u_select_related).filter(_followed_users)
    ...
    frends = Post.objects.only('created').select_related(*u_select_related).filter(_frends_query)
