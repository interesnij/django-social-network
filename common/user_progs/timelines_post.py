...
def get_timeline_posts(user):
    ...
    my_posts = user.post_creator.select_related(*select_related).only('created').filter(Q(creator_id=user.pk, community__isnull=True, is_deleted=False, status="P"))
    ...
    community_posts = Post.objects.select_related(*select_related).only('created').filter(_community_posts)
    ...
    frends = Post.objects.select_related(*select_related).only('created').filter(_frends_query)
