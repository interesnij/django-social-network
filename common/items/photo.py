...
def c_photo(user, value):
    try:
        from gallery.models import Photo
        photo = Photo.objects.get(pk=item[3:], is_public=True)
        return ''.join([block, '<div class="photo"><div class="progressive replace image_fit c_' + photo.get_type() + '_photo pointer" data-href="', photo.file.url, '" photo-pk="', str(photo.pk), '"><img class="preview image_fit" width="20" height="15" loading="lazy" src="', photo.preview.url,'" alt="img"></div></div>'])
    except:
        pass
...
def get_photo(user, notify):
    if notify.verb == "ITE":
        ...
        if notify.is_have_object_set():
            ...
            if notify.community:
                for img in set:
                    photos = ''.join([photos, c_photo(user, notify.attach[3:])])
                ...
                for img in set:
                    photos = ''.join([photos, u_photo(user, notify.attach[3:])])
        ...
        if notify.is_have_object_set():
            ...
            first_notify.creator.get_full_name() + '</a> и ещё ' + str(notify.count_object_set()) + first_notify.get_verb_display()\
             + ' фотографию </p>' + post(user, notify.attach[3:])
        else:
