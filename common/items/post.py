...
def post(user, value):
    try:
        ...
        if post.community:
            ...
            return ''.join([block, '<span data-pk="', str(community.pk), '"list-pk="', str(post.list.pk), '" data-uuid="', str(post.uuid), '"><div class="card-header">\
            ...
            if post.creator.pk == user.pk:
                ...
                card_drop = '<span class="dropdown-item post_claim">Пожаловаться</span>'
            return ''.join([block, '<span data-pk="', str(creator.pk), '"list-pk="', str(post.list.pk), '" data-uuid="', str(post.uuid), '"><div class="card-header">\
