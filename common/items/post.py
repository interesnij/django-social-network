...
def post(user, value):
    try:
        ...
        if post.community:
            ...
            return ''.join([block, '<div class="pag card mb-3" data-type="community_post" data-pk="', str(post.pk), '"><div class="card-header">\
            ...
            </svg>', str(post.all_visits_count()), '</span></div><div class="load_comments"></div></div></div>'])
            ...
            if post.creator.pk == user.pk:
                ...
                card_drop = '<span class="dropdown-item post_claim">Пожаловаться</span>'
            return ''.join([block, '<div class="pag card mb-3" data-type="user_post" data-pk="', str(post.pk), '"><div class="card-header">\
