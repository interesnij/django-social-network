...
def user_post_notify(creator, recipient_id, action_community_id, post_id, comment_id, socket_name, verb):
    ...
    if action_community_id:
        ...
        if PostNotify.objects.filter(creator=creator, post_id=post_id, community__isnull=True, verb=current_verb).exists():
            ...
            PostNotify.objects.create(creator=creator, recipient_id=recipient_id, post_id=post_id, verb=current_verb, user_set=notify)
        elif PostNotify.objects.filter(recipient_id=recipient_id, community__isnull=True, created__gt=today, post_id=post_id, verb__in=verb).exists():
            notify = PostNotify.objects.get(recipient_id=recipient_id, community__isnull=True, post_id=post_id, created__gt=today, verb__in=verb)
