...
urlpatterns = [
    ...
    url(r'^u_comment_like_window/(?P<comment_pk>\d+)/(?P<pk>\d+)/$', ItemUserCommentLikeWindow.as_view()),
	url(r'^u_comment_dislike_window/(?P<comment_pk>\d+)/(?P<pk>\d+)/$', ItemUserCommentDislikeWindow.as_view()),
