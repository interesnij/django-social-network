...
class SwitchView(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		...
		try:
			self.user, user_agent, user_pk, r_user_pk = User.objects.get(link=self.kwargs["link"]), request.META['HTTP_USER_AGENT'], int(self.kwargs["pk"]), request.user.pk
			if request.user.is_authenticated:
			...
			self.c, user_agent, c_pk, u_pk = Community.objects.get(link=self.kwargs["link"]), request.META['HTTP_USER_AGENT'], int(self.kwargs["pk"]), request.user.pk
			...
			self.template_name = get_detect_main_template("about/about.html", request.user, request.META['HTTP_USER_AGENT'])
		if MOBILE_AGENT_RE.match(user_agent):
		...
		def get_context_data(self, **kwargs):
			...
			c['user'],c['community'],c['fix_list'],c['photo_album'],c['video_album'],c['playlist'], c['docs_list'],c['good_album'],c['get_buttons_block'],c['common_frends'],c['common_friends_count'] = self.user, self.c, self.fix_list(), self.photo_album(), self.video_album(), self.playlist(), self.doc_list(), self.good_album(), self.get_buttons_block, self.common_frends, self.common_friends_count
			return c
