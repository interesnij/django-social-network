...
class ItemComment(models.Model):
    ...
    def get_likes_for_comment_item(self, user):
        ...
        return ItemCommentVotes.objects.filter(item_id=self.pk, vote__gt=0).filter(reactions_query)

    def get_dislikes_for_comment_item(self, user):
        reactions_query = user._make_get_votes_query_comment(comment=self)
        return ItemCommentVotes.objects.filter(item_id=self.pk, vote__lt=0).filter(reactions_query)
