...
class User(AbstractUser):
    ...
    def is_have_deleted_support_chats(self):
        from chat.models import Chat, ChatUsers
        return Chat.objects.filter(creator_id=self.pk, type__contains="_SU").exists()
    def get_deleted_support_chats(self):
        from chat.models import Chat, ChatUsers
        return Chat.objects.filter(creator_id=self.pk, type__contains="_SU")
    ...
    def is_support(self):
        ...
        return SupportUsers.objects.filter(manager=self.pk).exists()
    ...
    def get_online(self):
        ...
        naive = self.last_activity.replace(tzinfo=None)
        return datetime.now() < naive + timedelta(minutes=3)
    ...
    def get_online_display(self):
        ...
        naive = self.last_activity.replace(tzinfo=None)
        if datetime.now().timestamp() < naive + timedelta(minutes=3):
    ...
    def get_online_status(self):
        from datetime import datetime, timedelta
        naive = self.last_activity.replace(tzinfo=None)
        if datetime.now() < naive + timedelta(minutes=3):
            return '<i>Онлайн</i>'
        else: 
            if self.is_women():
    ...
    def get_user_notify(self):
        ...
        query = Q(creator_id__in=self.get_user_main_notify_ids())| \
                Q(community_id__in=self.get_community_main_notify_ids())
