...
class User(AbstractUser):
    ...
    def get_unread_chats(self):
        ...
        for chat in chats:
            if chat.chat_message.filter(unread=True, type__contains="_", message_options__user_id=user_id, message_options__is_deleted=True).exclude(creator_id=self.pk).exists():
                count += 1
        if count > 0:
            return count
        else:
