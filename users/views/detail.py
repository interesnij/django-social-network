...
class UserPostView(TemplateView):
    ...
    def get_context_data(self,**kwargs):
        ...
        c["object"], c["user"], c["next"], c["prev"] = self.post, self.list.user, self.posts.filter(pk__gt=self.post.pk, is_delete=True).order_by('pk').first(), self.posts.filter(pk__lt=self.post.pk, is_delete=True).order_by('-pk').first()
        return c
