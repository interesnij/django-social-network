...
class ItemListView(View, EmojiListMixin):
    ...
    def get(self, request, **kwargs):
        ...
        if self.user != request.user:
            check_is_not_blocked_with_user_with_id(user=self.user, user_id=request.user.id)
            if self.user.is_closed_profile:
                check_is_connected_with_user_with_id(user=self.user, user_id=request.user.id)
