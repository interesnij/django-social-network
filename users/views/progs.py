...
class GetUserGender(View):
    def get(self,request,*args,**kwargs):
        if request.user.gender:
            ...
            dfru = pd.read_csv('http://трезвый.рус/static/scripts/csv/FNru.csv', encoding='utf-8', quotechar='"', delimiter=';')
            dfen = pd.read_csv('http://трезвый.рус/static/scripts/csv/FNen.csv', encoding='utf-8', quotechar='"', delimiter=';')
