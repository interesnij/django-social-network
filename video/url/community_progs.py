...
urlpatterns = [
    ...
    url(r'^add_video_in_list/(?P<pk>\d+)/(?P<video_pk>\d+)/$', AddVideoInCommunityList.as_view()),
    url(r'^remove_video_from_list/(?P<pk>\d+)/(?P<video_pk>\d+)/$', RemoveVideoFromCommunityList.as_view()),
