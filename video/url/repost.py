...
# u_ucm_video - видеозапись пользователя(u) к себе на стену(u), в сообщество (c) или в сообщения(m)
    ...
    url(r'^u_ucm_video_list_window/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', UUCMVideoListWindow.as_view()),
    url(r'^c_ucm_video_list_window/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', CUCMVideoListWindow.as_view()),
    ...
    url(r'^u_u_video_list_repost/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', UUVideoListRepost.as_view()),
    url(r'^c_u_video_list_repost/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', CUVideoListRepost.as_view()),
    url(r'^u_c_video_list_repost/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', UCVideoListRepost.as_view()),
    url(r'^c_c_video_list_repost/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', CCVideoListRepost.as_view()),
    url(r'^u_m_video_list_repost/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', UMVideoListRepost.as_view()),
    url(r'^c_m_video_list_repost/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', CMVideoListRepost.as_view()),
]
