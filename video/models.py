...
class VideoList(models.Model):
    ...
    def get_longest_penalties(self):
        ...
        return ModerationPenalty.objects.filter(type="VIL", object_id=self.pk)[0].get_expiration()
    ...
    def is_community_list(self, community):
        return self in community.get_video_lists()
    ...
    def add_in_user_collections(self, user):
        if (self.community or self.creator.pk != user.pk) not in [i['pk'] for i in self.users.exclude(type__contains="_").values("pk")]:
            from users.model.list import UserVideoListPosition
            self.users.add(user)
    ...
    def copy_item(pk, user_or_communities):
        item = VideoList.objects.get(pk=pk)
...
class Video(models.Model):
    ...
    def copy_item(pk, lists):
        ...
        for list_pk in lists:
            post_list = VideoList.objects.get(pk=list_pk)
            Video.create_post(
    ...
    def create_comment(self, commenter, attach, parent, text, sticker):
        ...
        if sticker:
            comment = VideoComment.objects.create(commenter=commenter, sticker_id=sticker, parent=parent, item=self)
        else:
            comment = VideoComment.objects.create(commenter=commenter, attach=_attach, parent=parent, item=self, text=get_text_processing(text))
        self.comment += 1
        self.save(update_fields=["comment"])
        if parent:
            if self.community:
                from common.notify.notify import community_notify, community_wall
                community_notify(comment.commenter, self.community, None, comment.pk, "VIDC", "u_video_comment_notify", "REP")
                community_wall(comment.commenter, self.community, None, comment.pk, "VIDC", "u_video_comment_notify", "REP")
            else:
            ...
            if self.community:
                from common.notify.notify import community_notify, community_wall
                community_notify(comment.commenter, self.community, None, comment.pk, "VIDC", "u_video_comment_notify", "COM")
                community_wall(comment.commenter, self.community, None, comment.pk, "VIDC", "u_video_comment_notify", "COM")
            else:
                from common.notify.notify import user_notify, user_wall
...
class VideoComment(models.Model):
    ...
    def send_like(self, user, community):
        ...
        if not item.votes_on or not item.list.is_user_can_see_comment(user.pk) or not item.list.is_user_can_see_el(user.pk):
            from django.http import Http404
            raise Http404
