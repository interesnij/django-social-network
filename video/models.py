...
class VideoList(models.Model):
    ...
    def get_can_see_el_exclude_users(self):
        ...
        query = VideoListPerm.objects.filter(list_id=self.pk, can_see_item=2).values("user_id")
        return User.objects.filter(id__in=[i['user_id'] for i in query])
    def get_can_see_el_include_users(self):
        from users.models import User
        query = VideoListPerm.objects.filter(list_id=self.pk, can_see_item=1).values("user_id")
        return User.objects.filter(id__in=[i['user_id'] for i in query])

    def get_can_see_comment_exclude_users(self):
        from users.models import User
        query = VideoListPerm.objects.filter(list_id=self.pk, can_see_comment=2).values("user_id")
        return User.objects.filter(id__in=[i['user_id'] for i in query])
    def get_can_see_comment_include_users(self):
        from users.models import User
        query = VideoListPerm.objects.filter(list_id=self.pk, can_see_comment=1).values("user_id")
        return User.objects.filter(id__in=[i['user_id'] for i in query])

    def get_create_el_exclude_users(self):
        from users.models import User
        query = VideoListPerm.objects.filter(list_id=self.pk, create_item=2).values("user_id")
        return User.objects.filter(id__in=[i['user_id'] for i in query])
    def get_create_el_include_users(self):
        from users.models import User
        query = VideoListPerm.objects.filter(list_id=self.pk, create_item=1).values("user_id")
        return User.objects.filter(id__in=[i['user_id'] for i in query])

    def get_create_comment_exclude_users(self):
        from users.models import User
        query = VideoListPerm.objects.filter(list_id=self.pk, create_comment=2).values("user_id")
        return User.objects.filter(id__in=[i['user_id'] for i in query])
    def get_create_comment_include_users(self):
        from users.models import User
        query = VideoListPerm.objects.filter(list_id=self.pk, create_comment=1).values("user_id")
        return User.objects.filter(id__in=[i['user_id'] for i in query])

    def get_copy_el_exclude_users(self):
        from users.models import User
        query = VideoListPerm.objects.filter(list_id=self.pk, can_copy=2).values("user_id")
        return User.objects.filter(id__in=[i['user_id'] for i in query])
    def get_copy_el_include_users(self):
        from users.models import User
        query = VideoListPerm.objects.filter(list_id=self.pk, can_copy=1).values("user_id")
        return User.objects.filter(id__in=[i['user_id'] for i in query])
