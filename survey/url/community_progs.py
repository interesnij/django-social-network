...
urlpatterns=[
    ...
    url(r'^abort_delete/(?P<pk>\d+)/(?P<survey_pk>\d+)/$', SurveyCommunityAbortDelete.as_view()),
    ...
    url(r'^abort_delete_list/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', CommunitySurveyListAbortDelete.as_view()),
