...
on('#ajax', 'click', '.c_ucm_music_repost', function() {
  ...
  create_fullscreen("/music/repost/c_ucm_music_window/" + pk + "/" + track_pk + "/", "item_fullscreen");
  clear_attach_block();
})
on('#ajax', 'click', '.c_ucm_music_list_repost', function() {
  ...
  create_fullscreen("/music/repost/c_ucm_list_window/" + pk + "/" + uuid + "/", "item_fullscreen");
  clear_attach_block();
})

on('#ajax', 'click', '.c_soundcloud_set_create', function() {
  pk = document.body.querySelector(".pk_saver").getAttribute("data-pk");
  create_fullscreen("/music/community_progs/souncloud_create_list_window/" + pk + "/", "item_fullscreen");
});

on('#ajax', 'click', '.c_soundcloud_set_list', function() {
  uuid = this.parentElement.parentElement.getAttribute('data-uuid');
  create_fullscreen("/music/community_progs/souncloud_list_window/" + uuid + "/", "item_fullscreen");
});
on('#ajax', 'click', '.c_music_list_create_window', function() {
  pk = document.body.querySelector(".pk_saver").getAttribute("data-pk");
  create_fullscreen("/music/community_progs/create_list_window/" + pk + "/", "item_fullscreen");
});

on('#ajax', 'click', '.c_playlist_edit', function() {
  uuid = this.parentElement.parentElement.getAttribute('data-uuid');
  create_fullscreen("/music/community_progs/edit_list/" + uuid + "/", "item_fullscreen");
});

on('#ajax', 'click', '.c_track_add', function() {
  create_fullscreen("/music/community_progs/add_track/", "item_fullscreen");
});
on('#ajax', 'click', '.c_playlist_add', function() {
  create_fullscreen("/music/community_progs/add_list/" + document.body.querySelector(".pk_saver").getAttribute('data-pk') + "/", "item_fullscreen");
});
