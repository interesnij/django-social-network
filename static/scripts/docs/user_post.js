...
on('#ajax', 'click', '.u_add_doc_in_list', function() {
  ...
  if ( link.readyState == 4 && link.status == 200 ) {
    ...
    list.classList.add("u_remove_doc_from_list");
...
on('#ajax', 'click', '.u_remove_doc_from_list', function() {
  ...
  pk = _this.parentElement.parentElement.parentElement.parentElement.getAttribute("data-pk");
  link = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject( 'Microsoft.XMLHTTP' );
  link.open( 'GET', '/docs/user_progs/u_remove_doc_from_list/' + pk + "/" + uuid + "/", true );
  link.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
  link.onreadystatechange = function () {
  if ( link.readyState == 4 && link.status == 200 ) {
    list = parent.querySelector(".u_remove_doc_from_list");
    list.style.paddingLeft = "30px";
    list.classList.add("u_add_doc_in_list");
    list.classList.remove("u_remove_doc_from_list");
