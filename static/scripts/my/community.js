...
$('#ajax').on('click', '.item_community_unfixed', function() {var link = $(this).parent(); var pk = unfixed.parent().data('id'); var uuid = unfixed.parent().data('uuid');$.ajax({url: "/community/unfixed/" + pk + "/" + uuid + "/",success: function(data) {link.parent().html("<span style='cursor:pointer' class='dropdown-item item_community_fixed'>Закрепить</span>");$.toast({heading: 'Информация',text: 'Запись откреплена!',showHideTransition: 'fade',icon: 'info'})}});});
