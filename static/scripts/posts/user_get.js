...
on('#ajax', 'click', '.u_item_comments', function() {
  ...
  block.classList.contains("show") ? (block.style.display == "none", block.classList.remove("show")) : (list_load(block, "/posts/user/comment/" + uuid + "/" + pk + "/"), block.classList.add("show"))
});
