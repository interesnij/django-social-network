...
text = 'Группа ВК «ЗаДобровольность»: https://vk.com/zadobrovolnost<br><br>#ЗаДобровольность #Вакцинация #Коронавирус'
    ...
    for word in words:
        ...
        if "#" in word:
            ...
            if word[0] == "#":
                _p = word.strip(".,:;!_*-+()/@#¤%&)")
                tag = "#" + _p
                _loop[next] = _loop[this].replace(tag + indent, '<a class="ajax action"href="/search/?tag=' + _p + '">' + tag + '</a>' + indent)
            else:
                _p = word.strip(".,:;!_*-+()/@#¤%&)")
                p_2 = _p[_p.find("#") + 1:]
                tag = "#" + p_2
