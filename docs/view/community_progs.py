...
class CommunityDocCreate(TemplateView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        if request.is_ajax() and form_post.is_valid() and request.user.is_administrator_of_community(self.kwargs["pk"]):
            ...
            new_doc = doc.create_doc(creator=request.user,title=doc.title,file=doc.file,lists=request.POST.getlist("lists"),is_public=request.POST.get("is_public"),community=Community.objects.get(pk=self.kwargs["pk"]))
