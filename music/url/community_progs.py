...
urlpatterns = [
    ...
    url(r'^add_track_in_list/(?P<pk>\d+)/(?P<list_pk>\d+)/$', AddTrackInCommunityList.as_view()),
    url(r'^remove_track_from_list/(?P<pk>\d+)/(?P<list_pk>\d+)/$', RemoveTrackFromCommunityList.as_view()),
