...
urlpatterns = [
    ...
    url(r'^restore_list/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', CommunityPlaylistAbortDelete.as_view()),
    ...
    url(r'^remove_track_in_list/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', RemoveTrackInCommunityList.as_view()),
    url(r'^create/(?P<pk>\d+)/$', CommunityTrackCreate.as_view()),
    url(r'^edit/(?P<pk>\d+)/(?P<track_pk>\d+)/$', CommunityTrackEdit.as_view()),
    url(r'^delete/(?P<pk>\d+)/(?P<track_pk>\d+)/$', CommunityTrackRemove.as_view()),
    url(r'^restore/(?P<pk>\d+)/(?P<track_pk>\d+)/$', CommunityTrackAbortRemove.as_view()),
]
