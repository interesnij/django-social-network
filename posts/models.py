            ...
            if item[:3] == "pho":
                ...
                if user.is_authenticated:
                    ...
                    if playlist.is_user_can_add_list(creator.pk):
                        add_svg = '<a class="u_add_music_list"><svg fill="currentColor" class="svg_default" style="width: 17px;" viewBox="0 0 24 24"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/><path d="M0 0h24v24H0z" fill="none"/></svg></a>'
                    elif creator.pk != user.pk:
                        add_svg = '<a class="u_remove_music_list"><svg fill="currentColor" class="svg_default" style="width: 17px;" viewBox="0 0 24 24"><path fill="none" d="M0 0h24v24H0z"/><path d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z"/></svg></a>'
                block = ''.join([block, '<div style="flex-basis: 100%;" class="card" data-pk="', str(creator.pk), '" data-uuid="', str(playlist.uuid), '"><div class="card-body" style="padding: 8px;padding-bottom: 0;"><div style="display:flex"><figure><a class="u_load_music_list pointer">', image, '</a></figure><div class="media-body" style="margin-left: 10px;"><h6 class="my-0 mt-1 u_load_music_list pointer">', playlist.name, '</h6><p>Плейлист <a class="ajax underline" href="', creator.get_link(), '">', creator, '</a><br>Треков: ', str(playlist.count_tracks()), '</p></div>', add_svg, repost_svg, '</div></div></div>'])
