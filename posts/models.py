        ...
        for item in self.attach.split(","):
            if item[:3] == "pho":
                ...
                try:
                    ...
                    block = ''.join([block, '<div data-pk="', str(album.creator.pk), '" data-uuid="', str(album.uuid), '" style="padding: 7px;"><div class="media mb-2"><div class="media-body"><h4 class="content-color-primary mb-0"><a class="u_load_good_album pointer">', album.title, '</a></h4></div><span class="small">', share, add, '</span></div><div class="align-items-center no-gutters"><figure class="mx-auto mb-3" style="width:120px"><img class="u_load_good_album pointer image_fit_small" src="', album.get_cover, '" style="border-radius:50%" /></figure></div><h5 class="card-title mb-2 header-color-primary text-center"><a href="', creator.get_link(), '" class="ajax underline">', creator.get_full_name(), '</a></h5><h6 class="card-subtitle header-color-secondary text-center">', album.count_goods_ru(), '</h6></div>'])
                except:
                    pass
