        ...
        for item in self.attach.split(","):
            if item[:3] == "pho":
                ...
                try:
                    ...
                    if user.is_authenticated:
                        if album.is_not_empty():
                            share = '<a class="btn btn-sm primary-gradient u_ucm_good_album_repost"><svg fill="#ffffff" style="width: 17px;" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"></path><path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81 1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3c0 .24.04.47.09.7L8.04 9.81C7.5 9.31 6.79 9 6 9c-1.66 0-3 1.34-3 3s1.34 3 3 3c.79 0 1.5-.31 2.04-.81l7.12 4.16c-.05.21-.08.43-.08.65 0 1.61 1.31 2.92 2.92 2.92 1.61 0 2.92-1.31 2.92-2.92s-1.31-2.92-2.92-2.92z"></path></svg></a>'
                        if album.is_user_can_add_list(user.pk):
                    ...
                    block = ''.join([block, '<div data-pk="', str(creator.pk), '" data-uuid="', str(album.uuid), '" style="padding: 7px;width: 100%;flex-basis: 100%"><div class="media mb-2"><div class="media-body"><h4 class="content-color-primary mb-0"><a class="u_load_good_album pointer">', album.title, '</a></h4></div><span class="small">', share, add, '</span></div><div class="align-items-center no-gutters"><figure class="mx-auto mb-3" style="width:120px"><img class="u_load_good_album pointer image_fit_small" src="', album.get_cover(), '" style="border-radius:50%" /></figure></div><h5 class="card-title mb-2 header-color-primary text-center"><a href="', creator.get_link(), '" class="ajax underline">', creator.get_full_name(), '</a></h5><h6 class="card-subtitle header-color-secondary text-center">', album.count_goods_ru(), '</h6></div>'])
                except:
                    pass
