...
class PostsList(models.Model):
    ...
    def get_longest_penalties(self):
        return self.manager_penalties.filter(type="POL", object_id=self.pk)[0].expiration
    ...
    def get_description(self):
        if self.community:
            return 'список записей сообщества <a href="' + self.community.get_link() + '" target="_blank">' + self.community.name + '</a>'
        else:
    ...
    def is_community_list(self, community):
        return self in community.get_post_lists()
    ...
    def add_in_community_collections(self, community):
        if (self.community and self.community.pk != community.pk) or community.pk not in [i['pk'] for i in self.communities.exclude(type__contains="_").values("pk")]:
            from communities.model.list import CommunityPostsListPosition
    ...
    def remove_in_community_collections(self, community):
        if (self.community and self.community.pk != community.pk) or community.pk in [i['pk'] for i in self.communities.exclude(type__contains="_").values("pk")]:
            from communities.model.list import CommunityPostsListPosition
            try:
...
class Post(models.Model):
    ...
    def get_description(self):
        if self.community:
            return 'запись сообщества <a href="' + self.community.get_link() + '" target="_blank">' + self.community.name + '</a>'
        else:
...
class PostComment(models.Model):
    ...
    def get_replies(self):
        return self.replies.filter(Q(type=PostComment.EDITED)|Q(type=PostComment.PUBLISHED))
