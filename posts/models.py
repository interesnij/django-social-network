        ...
        for item in self.attach.split(","):
            if item[:3] == "pho":
                ...
                try:
                    ...
                    block = ''.join([block, '<div style="flex: 0 0 100%;" survey-pk="', str(survey.pk), '" data-pk="', str(survey.creator.pk), '" class="border text-center has-background-img position-relative box-shadow"><figure class="background-img">', image, '</figure><div class="container" style="list-style-type:none"><i class="figure avatar120 mr-0 fa fa-gift rounded-circle bg-none border-bottom"></i><br><h4 class="u_survey_detail pointer">', survey.title, '</h4><a class="underline ajax" href="', survey.creator.get_links(), '">', str(survey.creator), '</a>', time, '<br>', answers, voters, '</span></div></div>'])
                except:
                    pass
            elif item[:3] == "lmu":
                try:
