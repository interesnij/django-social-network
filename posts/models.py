        ...
        for item in self.attach.split(","):
            if item[:3] == "pho":
                ...
                try:
                    ...
                    block = ''.join([block, '<div class="custom_color text-center has-background-img position-relative box-shadow" data-pk="', str(creator.pk), '" data-uuid="', str(album.uuid), '" style="width: 100%;flex-basis: 100%;"><figure class="background-img"><img src="', album.get_cover_photo().file.url, '">"</figure><div class="container"><i class="figure avatar120 mr-0 fa fa-gift rounded-circle bg-none"></i><br><h4 class="u_load_photo_album pointer">', album.title, '</h4><p class="lead"><a class="ajax underline" href="', creator.get_link(), '">', str(album.creator), '</a></p><hr class="my-3"><p>', album.count_photo_ru(), '<div class="row">', share, add, '</div></p>', '</div></div>'])
                except:
                    pass
