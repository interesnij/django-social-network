...
urlpatterns = [
    ...
    url(r'^comment/(?P<pk>\d+)/like/$',login_required(VotesView.as_view(model=PostComment2, vote_type=LikeDislike.LIKE)),name='post_comment_like'),
    url(r'^comment/(?P<pk>\d+)/dislike/$',login_required(VotesView.as_view(model=PostComment2, vote_type=LikeDislike.DISLIKE)),name='post_comment_dislike'),
