...
class PostUserDetail(TemplateView):
    ...
    def get(self,request,*args,**kwargs):
        ...
        if request.user.is_authenticated:
            self.template_name = get_template_user_item(self.post, "posts/post_user/", "detail.html", request.user, request.META['HTTP_USER_AGENT'], request.GET.get("stat"))
        else:
...
class PostLoadView(TemplateView):
    ...
    def get(self,request,*args,**kwargs):
        self.post = Post.objects.get(uuid=self.kwargs["uuid"])
        self.template_name = get_permission_user_post_2(self.post.creator, "posts/post_user/", "post.html", request.user, request.META['HTTP_USER_AGENT'], request.GET.get("stat"))
