    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.is_ajax() and form_post.is_valid():
            ...
            if parent.parent:
                ...
                parent = parent
            for object_id in connections:
                new_post = post.create_post(creator=request.user, list=parent.list, text=post.text, category=None, attach=attach, parent=parent, is_signature=False, comments_enabled=, votes_on=False, community=None)
                if object_id[0] == "c":
                    chat = Chat.objects.get(pk=object_id[1:])
