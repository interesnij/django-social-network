...
urlpatterns = [
	...
	url(r'^restore/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', PostCommunityRecover.as_view()),
	url(r'^wall_restore/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', PostWallCommunityRecover.as_view()),
    ...
    url(r'^restore_list/(?P<pk>\d+)/(?P<list_pk>\d+)/$', CommunityPostListRecover.as_view()),
