...
urlpatterns = [
	...
	url(r'^restore_comment/(?P<pk>\d+)/$', PostCommentUserAbortDelete.as_view()),
	url(r'^delete_wall_comment/(?P<pk>\d+)/(?P<comment_pk>\d+)/$', PostWallCommentUserDelete.as_view()),
	url(r'^restore_wall_comment/(?P<pk>\d+)/(?P<comment_pk>\d+)/$', PostWallCommentUserAbortDelete.as_view()),
	...
	url(r'^restore/(?P<uuid>[0-9a-f-]+)/$', PostUserAbortDelete.as_view()),
	url(r'^wall_restore/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', PostWallUserAbortDelete.as_view()),
    ...
    url(r'^restore_list/(?P<list_pk>\d+)/$', UserPostListAbortDelete.as_view()),
