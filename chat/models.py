...
class Chat(models.Model):
    ...
    def get_unread_count_message(self, user_id):
        count = self.chat_message.filter(unread=True, type__contains="_", message_options__user_id=user_id, message_options__is_deleted=True).exclude(creator_id=user_id).values("pk").count()
        if count:
