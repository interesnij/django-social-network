...
class Chat(models.Model):
    ...
    def get_header_private_chat(self, user_id):
        ...
        if self.name:
            ...
            chat_name = member.get_full_name()
        media_body = ''.join(['<div class="media-body" style="overflow: inherit;"><h5 class="time-title mb-0 pointer u_chat_settings">', chat_name, '</h5><p class="mt-1 target_display"><span class="type_display">', member.get_online_status(), '</span>', buttons, '</p></div>'])
    ...
    def get_header_group_chat(self, user_id):
        ...
        if self.name:
            ...
            chat_name = "Групповой чат"
        media_body = ''.join(['<div class="media-body" style="overflow: inherit;"><h5 class="time-title mb-0 pointer u_chat_settings">', chat_name, '</h5><p class="mt-1 target_display"><span class="type_display">', self.get_members_count_ru(), '</span>', buttons, '</p></div>'])
    ...
    def get_or_create_chat_and_send_message(creator, user, repost, text, attach, voice, sticker):
        ...
        for recipient_id in current_chat.get_recipients_ids(creator.pk):
            if voice:
                ...
                recipient_message = Message.objects.create(chat=current_chat, copy=creator_message, sticker_id=sticker, recipient_id=recipient_id, repost=repost, voice=voice, type=Message.PROCESSING)
            else:
