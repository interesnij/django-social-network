...
urlpatterns=[
	...
	url(r'^restore_comment/(?P<pk>\d+)/$', login_required(GoodCommentCommunityAbortDelete.as_view())),
    ...
    url(r'^restore/(?P<pk>\d+)/(?P<good_pk>\d+)/$', CommunityGoodAbortDelete.as_view()),
    ...
    url(r'^restore_list/(?P<pk>\d+)/(?P<uuid>[0-9a-f-]+)/$', CommunityGoodListAbortDelete.as_view()),
