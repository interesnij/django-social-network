...
class GoodList(models.Model):
	...
	def get_longest_penalties(self):
		...
		return ModerationPenalty.objects.filter(type="GOL", object_id=self.pk)[0].get_expiration()
	...
	def get_description(self):
		if self.community:
			return 'подборку товаров сообщества <a href="' + self.community.get_link() + '" target="_blank">' + self.community.name + '</a>'
		else:
	...
	def copy_item(pk, user_or_communities):
		item = GoodList.objects.get(pk=pk)
...
class Good(models.Model):
	...
	def get_description(self):
		if self.community:
			return 'товар сообщества <a href="' + self.community.get_link() + '" target="_blank">' + self.community.name + '</a>'
		else:
	...
	def copy_item(pk, lists):
		item, count = Good.objects.get(pk=pk), 0
		for list_pk in lists:
			post_list = GoodList.objects.get(pk=list_pk)
			good = Good.create_photo(
